...
export class FriendsEffects {
  loadFriends$ = createEffect(() =>
    this.actions$.pipe(
      ofType(AtomicStatePageActions.enter),
      switchMap(() => {
        return this.friendsApiService.list().pipe(
          map((friends) => {
            return FriendsApiActions.loadSuccess({ friends });
          }),
          catchError(() =>
            of(
              FriendsApiActions.loadFailure({
                error:
                  'Failed loading the list of friends. Please contact your administrator'
              })
            )
          )
        );
  ...
  addFriend$ = createEffect(() =>
    this.actions$.pipe(
      ofType(AtomicStatePageActions.addFriend),
      concatMap(({ friend }) => {
        return this.friendsApiService.add(friend).pipe(
          map((newFriend) => {
            return FriendsApiActions.addFriendSuccess({ friend: newFriend });
          }),
          catchError((error) =>
            of(
              FriendsApiActions.addFriendFailure({
                error:
                  'Failed adding a friend. Please contact your administrator'
              })
            )
          )
        );
